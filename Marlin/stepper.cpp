...
/*
 * Y axis step-skip protection
 *
 * AS5048 get_output()/2 can't ever be 255, guaranteed
 * to be different first time
 */
...
bool check_rotation(void) {
    uint8_t rotationval;

    rotationval = as5048_read(&mre);
    if (rotationval == last_pos) {
        return true;
    }
    else {
        last_pos = rotationval;
        return false;
    }
    return false;
}
