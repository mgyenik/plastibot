...
/*
 * Code for I2C version of AS5048 magnetic rotary encoder
 */
#include <Wire.h>
...
//Servo slideservo; /* Servo for scraper actuation */
#define SERVOPIN 4
#define RETRACT_END_SWITCH 58
#define RETRACT_START_SWITCH 57

#define SERVODELAY(__us) \
    digitalWrite(SERVOPIN, HIGH); \
    delayMicroseconds(__us); \
    digitalWrite(SERVOPIN, LOW); \
    delayMicroseconds(20000 - __us);
...
uint8_t as5048_read(struct as5048b *a) {
    uint16_t ang_int;
    uint8_t lo;
    uint8_t hi;

    Wire.beginTransmission(a->addr);
    Wire.write(AS5048B_ANGLEH);
    Wire.endTransmission();
    
    Wire.requestFrom(a->addr, (uint8_t)2);
    lo = Wire.read();
    hi = Wire.read();

    ang_int = (hi << 6) | (lo & 0x3f);
    return ang_int >> 8;
}

void setup()
{ 
  //slideservo.attach(4);                 /* attached to digital pin 4 */
  //slideservo.writeMicroseconds(1425);   /* 1425us is hold position */

  pinMode(SERVOPIN, OUTPUT);
  pinMode(RETRACT_END_SWITCH, INPUT);
  pinMode(RETRACT_START_SWITCH, INPUT);
  digitalWrite(RETRACT_END_SWITCH, HIGH);
  digitalWrite(RETRACT_START_SWITCH, HIGH);
  Wire.begin();
  setup_powerhold();
...
/*
 * Servo movements for ejector
 */
static void retract_ejector(void)
{
    while (digitalRead(RETRACT_START_SWITCH) == HIGH) {
        digitalWrite(SERVOPIN, HIGH);
        delayMicroseconds(1900);
        digitalWrite(SERVOPIN, LOW);
        delayMicroseconds(20000 - 1900);
    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        digitalWrite(SERVOPIN, LOW);
        delayMicroseconds(20000 - 1390);
    }
}

static void extend_ejector(void)
{
    while (digitalRead(RETRACT_END_SWITCH) == LOW) {
        digitalWrite(SERVOPIN, HIGH);
        delayMicroseconds(1100);
        digitalWrite(SERVOPIN, LOW);
        delayMicroseconds(20000 - 1100);
    }
...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
    ...
    case 40:
        ...
        /* Move Z out of the way and Y to fully extended */
        destination[X_AXIS] = current_position[X_AXIS];
        destination[Y_AXIS] = 190.0f;
        destination[Z_AXIS] = current_position[Z_AXIS];
        ...
        /* Get Y into cleared zone */
        destination[X_AXIS] = current_position[X_AXIS];
        destination[Y_AXIS] = 80.0f;
        destination[Z_AXIS] = current_position[Z_AXIS];
        destination[E_AXIS] = current_position[E_AXIS];
        prepare_move();
...
void prepare_move()
{
    if (min_software_endstops) {
        if (destination[X_AXIS] < X_HOME_POS)
            destination[X_AXIS] = X_HOME_POS;

        if (destination[Y_AXIS] < Y_HOME_POS)
            destination[Y_AXIS] = Y_HOME_POS;

        if (destination[Z_AXIS] < Z_HOME_POS)
            destination[Z_AXIS] = Z_HOME_POS;
    }
    ...
    //    track_axis(&xtrack, destination[X_AXIS]);
